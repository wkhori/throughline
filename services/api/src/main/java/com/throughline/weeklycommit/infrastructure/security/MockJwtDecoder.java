 ...
 * strings checked into {@code apps/host/src/auth.ts}. In production, the deployed pipeline is Auth0
 * (real users) plus {@link DemoJwtDecoder} (demo personas via {@code /api/v1/auth/demo-login});
 * MockJwtDecoder is never instantiated.
 */
public class MockJwtDecoder implements JwtDecoder {
   ...
   * subs from scripts/auth0-provision.mjs), use it so the decoded sub matches the seeded user row.
   * Falls back to the deterministic stub sub used by Phase-1 dev/test.
   */
