...
- **MF contract:** Host and remote ship as two separate Vite 5 SPAs federated at runtime via `@module-federation/vite` 1.15.1. Host loads `weekly_commit_remote/App` from the remote's `remoteEntry.js` at `/app`. Singleton npm deps (React, Redux, RTK, react-redux, react-router-dom, Auth0) are shared via federation; workspace-source packages (`@throughline/shared-ui`, `@throughline/shared-types`) are bundled per-consumer. Both apps authenticate against the same Auth0 tenant — one shared JWT contract. Singleton versions pinned in `packages/shared-deps-versions.json`.
