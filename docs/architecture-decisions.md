...
| 34 | `@wkhori/evalkit` as the AI eval framework (CLAUDE.md §3 / `docs/ai-copilot-spec.md` §Eval Harness) | **Substitute** → inline minimal harness in `evals/runner.ts` | Package not on npm (P41). Substituted with a hand-rolled runner honouring the same contract: temperature 0, N=3, ≥2/3 pass, deterministic assertions (exact / contains / range / oneOf / maxLength / minLength / present / absent), per-touchpoint fixtures in `evals/fixtures/{t1..t7}/{input,expected}.json`. Runs against the real Anthropic API, writes `evals/last-run.md`, exits non-zero on any scenario failing the gate. Swap path: replace `evals/runner.ts` with a thin import of `@wkhori/evalkit` once published. |
