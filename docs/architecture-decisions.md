...
| 32 | AWS (EKS / CloudFront / S3 / SQS / SNS) | **Substitute** → Railway + Terraform in `infra/` | Demo runs on Railway for fast, reliable delivery. Production-grade Terraform skeleton in `infra/terraform/` (`terraform validate` clean) + Helm chart in `infra/helm/weekly-commit-api/` (`helm lint` clean) + `.github/workflows/aws-deploy.yml` placeholder. Swap path: `terraform apply` + ECR push + Helm install. Detail in `infra/README.md`. |
| 33 | Outlook Graph API integration | **Substitute** → Slack via `NotificationChannel` adapter | Adapter interface with `SlackChannel` (live), `OutlookGraphChannel` (stub), `LogChannel` (tests). Channel selected by config. Swap path: implement the stub, flip config. |
| 34 | `@wkhori/evalkit` as the AI eval framework | Implement (via published `evalkit` npm package) | `evals/runner.ts` wraps the published `evalkit` package: `evalkit.runChecks` handles the universal JSON-validity / schema / content checks. The runner adds a project-specific path-based assertion DSL (exact / contains / range / oneOf / maxLength / minLength / present / absent / allEqual / allEqualSet) for value and stability checks the package doesn't expose. Per-touchpoint fixtures in `evals/fixtures/{t1..t7}/{input,expected}.json`. Runs against the real Anthropic API, temperature 0, N=3, ≥2/3 pass; writes `evals/last-run.md`; exits non-zero on any scenario failing the gate. |
